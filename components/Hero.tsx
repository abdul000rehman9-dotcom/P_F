...
// 20 rich cards cycling along the wide 180-degree arch with video + image cards
const CARDS_DATA: CardItem[] = [
  ...
  { id: 9, title: "Web Development", img: "/images/card11.webp", type: "image" },
  { id: 10, title: "Product Design", img: "/images/card12.webp", type: "image" },
  { id: 11, title: "Creative direction", img: "/images/card1.webp", type: "image" },
  { id: 12, title: "UI/UX Strategy", img: "/images/card14.webp", type: "image" },
  { id: 13, title: "Visual Storytelling", img: "/images/img_3.webp", type: "image" },
  { id: 14, title: "Digital Craft", img: "/images/img_4.webp", type: "image" },
  { id: 15, title: "Brand Systems", img: "/images/img_5.webp", type: "image" },
  { id: 16, title: "Editorial Layouts", img: "/images/img_7.webp", type: "image" },
  { id: 17, title: "Dynamic Media", img: "/images/img_8.webp", type: "image" },
  { id: 18, title: "Creative Studio", img: "/images/img_6.webp", type: "image" },
  { id: 19, title: "Identity Design", img: "/images/img_11.webp", type: "image" },
  { id: 20, title: "Strategic Vision", img: "/images/img_12.webp", type: "image" },
];
...
export default function Hero() {
  ...
  // Scaled radius for wide 180-degree arch across screen sizes
  const getRadius = useCallback(() => {
    if (typeof window === "undefined") return 980;
    const w = window.innerWidth;
    if (w >= 1920) return 1300;
    if (w >= 1440) return 1150;
    if (w >= 1200) return 1020;
  ...
  const getCardWidth = useCallback(() => {
    ...
    const w = window.innerWidth;
    if (w >= 1920) return 240;
    if (w >= 1440) return 230;
    if (w >= 1200) return 215;
    if (w >= 1024) return 195;
    return 180;
  }, []);
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      // Dynamic angle step: ensures cards have a crisp, visible gap (~3-4px between corners) and never touch or overlap
      // Formula: 2 * arcsin((cardW + gapPx) / (2 * radius)) in degrees
      const gapPx = 26;
      ...
      cardRefs.current.forEach((el, index) => {
        ...
        el.style.transform = `translate3d(${x.toFixed(2)}px, ${y.toFixed(2)}px, 0px) translate(-50%, -50%) rotate(${angleDeg.toFixed(2)}deg)`;
      });
  ...
  return (
    ...
    >
      ...
      <div
        ref={arcContainerRef}
        className="hidden md:flex relative w-full overflow-visible mt-12 sm:mt-16 lg:mt-20 h-[220px] lg:h-[260px] xl:h-[290px] justify-center items-start z-10 pointer-events-none"
        aria-hidden="true"
      >
        {/* Arch apex reference point: sits lower with a clear gap below the launch button */}
        <div className="relative w-0 h-0 overflow-visible top-24 sm:top-28 lg:top-32">
          {CARDS_DATA.map((card, index) => (
            <div
              ...
              className="absolute left-0 top-0 will-change-transform pointer-events-auto transition-shadow duration-300"
              style={{
                width: "min(80vw, 250px)",
              }}
            >
              <div className="group w-[180px] lg:w-[215px] xl:w-[240px] rounded-[5px] bg-white p-1.5 lg:p-2 shadow-[0_6px_22px_rgba(0,0,0,0.06)] border border-black/10 transition-transform duration-300 ease-out hover:scale-105 hover:shadow-2xl hover:z-50 flex flex-col justify-between">
                <div className="aspect-[4/3] w-full overflow-hidden rounded-[3px] bg-neutral-100 relative">
                  {card.type === "video" ? (
                    <LazyVideo
                      src={card.img}
                    ...
                    <Image
                      ...
                      alt={card.title}
                      fill
                      sizes="(max-width: 1024px) 195px, 240px"
                ...
                <p className="mt-1 sm:mt-1.5 text-center font-sans text-[9.5px] lg:text-[10.5px] font-bold tracking-tight text-black uppercase truncate h-4 sm:h-5 flex items-center justify-center px-1">
                  {card.title}
                </p>
              </div>
            </div>
      ...
      <div className="block md:hidden relative z-20 mx-auto max-w-[540px] px-4 mt-8 pointer-events-auto">
        <div className="grid grid-cols-2 gap-3">
          {CARDS_DATA.slice(0, 4).map((card) => (
            <div
              key={card.id}
              className="group w-full rounded-[5px] bg-white p-1.5 shadow-[0_4px_16px_rgba(0,0,0,0.06)] border border-black/10 flex flex-col justify-between"
            >
              <div className="aspect-[4/3] w-full overflow-hidden rounded-[3px] bg-neutral-100 relative">
                {card.type === "video" ? (
                  <LazyVideo
              ...
              <p className="mt-1 text-center font-sans text-[9px] font-bold tracking-tight text-black uppercase truncate h-4 flex items-center justify-center px-1">
                {card.title}
              </p>
            </div>
          ))}
      ...
      {/* ========================================================================= */}
      {/* 3. ORANGE ASTERISK ICON & PARAGRAPH (REDUCED GAP TO CARDS)                */}
      {/* ========================================================================= */}
      <div className="relative z-20 mx-auto max-w-[1400px] px-6 text-center mt-3 sm:mt-4 pointer-events-auto">
        {/* Rotating Orange Asterisk Star Icon */}
        <motion.div
          initial={{ opacity: 0, scale: 0.8, y: 15 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          transition={{ duration: 0.75, delay: 0.35, ease: [0.22, 1, 0.36, 1] }}
          className="relative w-8 h-8 sm:w-9 sm:h-9 mx-auto pointer-events-none"
          aria-hidden="true"
        >
          <div className="w-full h-full relative animate-spin-continuous">
            <Image
              src="/icons/idotive-icon-3-orange.svg"
              alt="Orange Asterisk Star Icon"
              fill
        ...
        <motion.p
          ...
          className="text-xs sm:text-[13px] md:text-[13.5px] font-medium leading-relaxed text-neutral-600 max-w-md mx-auto mt-2 sm:mt-2.5"
        >
